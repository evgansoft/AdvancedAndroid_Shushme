...
public class MainActivity extends AppCompatActivity implements
    ...
    private void refreshPlacesData() {
        ...
        try {
            ...
            final List<String> ids  = new ArrayList<>(cursor.getCount());
            while (cursor.moveToNext()) {
                ids.add(cursor.getString(0));
            }
            final PendingResult<PlaceBuffer> placesPending = Places.GeoDataApi
                    .getPlaceById(client, ids.toArray(new String[ids.size()]));
            placesPending.setResultCallback(new ResultCallback<PlaceBuffer>() {
                @Override
